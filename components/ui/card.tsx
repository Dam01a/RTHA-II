...
function Card({ className, ...props }: React.ComponentProps<typeof View>) {
  return (
    <TextClassContext.Provider value="text-card-foreground">
      <View
        className={cn(
          'bg-card border-border/80 dark:border-border flex flex-col gap-5 rounded-2xl border py-5 shadow-sm shadow-black/5',
          className
        )}
        {...props}
      />
...
function CardHeader({ className, ...props }: React.ComponentProps<typeof View>) {
  return <View className={cn('flex flex-col gap-1 px-5', className)} {...props} />;
}
